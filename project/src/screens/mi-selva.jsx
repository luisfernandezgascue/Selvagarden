// MI SELVA — AI section + plant care

function MiSelva({ onTab }) {
  return (
    <Phone>
      <div className="scroll">
        ...
        {/* Avatar hero */}
        <div style={{
          margin:'18px 14px 0', padding:'28px 22px 22px',
          background:'linear-gradient(160deg, #F5EDD8 0%, #FBF6ED 80%)',
          borderRadius:20, textAlign:'center', position:'relative', overflow:'hidden',
        }}>
          <div style={{
            position:'absolute', inset:0,
            background:'radial-gradient(circle at 50% 0%, rgba(168,213,181,0.25), transparent 60%)',
          }}/>
        ...
        <div style={{ padding:'18px 14px 0', display:'flex', flexDirection:'column', gap:10 }}>
          <ActionCard
            icon={<Icon.Camera/>}
            title="Diagnosticar mi planta"
            sub="Sube una foto y te digo qué tiene"
            color="#1A3C2E"/>
          <ActionCard
            icon={<Icon.Leaf/>}
            title="Mis plantas"
            sub="4 plantas activas · 2 necesitan agua"
            color="#2D6A4F"
            badge="2"/>
          <ActionCard
            icon={<Icon.Play size={20}/>}
            title="Cuidados y videos"
            sub="Esta semana: poda de monstera"
            color="#B5873A"/>
        </div>
        ...
        <div style={{ padding:'0 18px', display:'flex', flexDirection:'column', gap:10 }}>
          <PlantRow
            ...
            sub="Comprada Nov 14 · Riego cada 7 días"
            img="https://images.pexels.com/photos/3097770/pexels-photo-3097770.jpeg?auto=compress&w=200"
            status="Próximo riego en 2 días"
            statusColor="#2D6A4F"/>
          <PlantRow
            name="Sansevieria Zeylanica"
            sub="Comprada Oct 28 · Riego cada 14 días"
            img="https://images.pexels.com/photos/2123482/pexels-photo-2123482.jpeg?auto=compress&w=200"
            status="Riego pendiente · ayer"
            statusColor="#B5873A"
            alert/>
          <PlantRow
            name="Pothos Marble Queen"
            sub="Comprada Sep 04 · Riego cada 10 días"
            img="https://images.pexels.com/photos/6913404/pexels-photo-6913404.jpeg?auto=compress&w=200"
            status="Próximo riego en 5 días"
            statusColor="#2D6A4F"/>
        </div>
        ...
        <div style={{ margin:'22px 14px 0', padding:'18px 18px', background:'#fff', border:'1px solid var(--c-line)', borderRadius:16, position:'relative', overflow:'hidden' }}>
          ...
          <h3 className="h-serif" style={{ fontSize:20, fontWeight:600, marginBottom:6, position:'relative' }}>
            ¿Viste un ramo que te gustó?
          </h3>
          <p style={{ fontSize:12, color:'#666', lineHeight:1.5, marginBottom:14, position:'relative' }}>
            Sube la foto y te lo hacemos, o te recomendamos algo similar.
          </p>
          <button style={{
            background:'#1A3C2E', color:'#F5EDD8', border:'none',
            borderRadius:'var(--r-btn)', padding:'10px 16px',
            fontSize:12, fontWeight:600, display:'flex', alignItems:'center', gap:8, position:'relative',
          }}>
      ...
      <div style={{
        ...
        {badge && (
          <span style={{
            ...
            background:'#B5873A', color:'#fff', fontSize:9, fontWeight:700,
            borderRadius:99, padding:'2px 6px', minWidth:16,
            border:'2px solid #F4F6F1',
          }}>{badge}</span>
        )}
      </div>
