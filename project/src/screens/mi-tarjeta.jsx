// MI TARJETA — QR loyalty card

function MiTarjeta({ storeMode = false, onTab }) {
  return (
    <Phone>
      <div className="scroll" style={{ padding:'8px 0 0' }}>
        ...
        {storeMode && (
          <div style={{
            ...
            borderRadius:13, display:'flex', alignItems:'center', gap:9,
          }}>
            <span style={{
              width:8, height:8, borderRadius:'50%', background:'#A8D5B5',
              boxShadow:'0 0 0 4px rgba(168,213,181,0.25)', animation:'pulseDot 1.6s infinite'
            }}/>
        ...
        <div style={{ padding:'18px 18px 14px', textAlign:'center' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
            ...
            <h2 className="h-serif" style={{ fontSize:30, fontWeight:500 }}>
              <span style={{ fontStyle:'italic' }}>Versailles</span>
            </h2>
          </div>
        ...
        {/* QR card */}
        <div style={{
          margin:'0 14px', padding: storeMode ? '24px 18px' : '20px 18px',
          background:'#fff', borderRadius:20,
          border:'1px solid var(--c-line)',
          boxShadow:'0 6px 30px rgba(26,60,46,0.08)',
        }}>
          <div style={{ display:'flex', justifyContent:'center', position:'relative' }}>
            <QRCode size={storeMode ? 260 : 220}/>
            {/* corner cuts decoration */}
            <CornerCuts size={storeMode ? 260 : 220}/>
          </div>
        ...
        {/* Wallet btn */}
        <button style={{
          margin:'18px 18px 0', background:'#1A1A1A', color:'#fff', border:'none',
          borderRadius:'var(--r-btn)', padding:'13px',
          fontSize:13, fontWeight:600, width:'calc(100% - 36px)',
...
function CornerCuts({ size }) {
  const s = 18;
  const wrap = { position:'absolute' };
  const piece = (orient) => ({
  ...
  return (
    <div style={{
      ...
      <span style={{
        ...
        color: gold ? '#B5873A' : '#1A3C2E',
      }}>{pts}</span>
    </div>
  );
}

Object.assign(window, { MiTarjeta });
