// PRODUCTO — product detail page

function Producto({ onBack }) {
  ...
  return (
    <Phone>
      {/* Sticky back/cart bar */}
      <div style={{
        position:'absolute', top:44, left:0, right:0,
        zIndex:20, padding:'8px 14px',
      ...
      <div className="scroll">
          ...
          <div style={{ position:'absolute', bottom:16, left:'50%', transform:'translateX(-50%)', display:'flex', gap:6 }}>
            {images.map((_, i) => (
              <button key={i} onClick={() => setGalleryIdx(i)} style={{
                width: i === galleryIdx ? 22 : 7, height:7, borderRadius:99,
                background: i === galleryIdx ? '#fff' : 'rgba(255,255,255,0.5)',
                border:'none', transition:'width .2s',
              }}/>
            ))}
          </div>
          {/* gallery thumbs */}
          <div style={{
            position:'absolute', left:14, bottom:48, display:'flex', flexDirection:'column', gap:6,
          }}>
            {images.map((src, i) => (
              <button key={i} onClick={() => setGalleryIdx(i)} style={{
                width:44, height:44, borderRadius:8, padding:0, overflow:'hidden',
                border: i === galleryIdx ? '2px solid #fff' : '2px solid rgba(255,255,255,0.4)',
                background:'#fff',
              }}>
        ...
        <div style={{ padding:'18px 18px 0', background:'#F4F6F1' }}>
          ...
          <div style={{
            margin:'14px 0 0', background:'#D8EDE3', border:'1px solid rgba(45,106,79,0.2)',
          ...
          <p style={{ fontFamily:'var(--font-serif)', fontSize:17, fontStyle:'italic', color:'#1A1A1A', lineHeight:1.5, marginBottom:10 }}>
            Una escultura viva.
          </p>
          ...
          {/* Especificaciones */}
          <div style={{
            background:'#fff', borderRadius:14, border:'1px solid var(--c-line)',
          ...
          {/* Combina con */}
          <p style={{ fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize:18, marginBottom:10 }}>
            Combina perfectamente con…
          </p>
          <div style={{ display:'flex', gap:10, overflowX:'auto', marginBottom:24, paddingBottom:4 }}>
            {weekProducts.slice(0,3).map((p, i) => (
      ...
      <div style={{
        ...
        background:'linear-gradient(180deg, transparent 0%, rgba(244,246,241,0.95) 30%, #F4F6F1 100%)',
      }}>
        <button style={{
          background:'#1A3C2E', color:'#fff', border:'none',
          borderRadius:'var(--r-btn)', padding:'15px',
          fontSize:14, fontWeight:600, width:'100%',
          display:'flex', alignItems:'center', justifyContent:'center', gap:10,
          boxShadow:'0 8px 22px rgba(26,60,46,0.25)',
        }}>
