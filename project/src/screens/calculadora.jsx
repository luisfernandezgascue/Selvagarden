// CALCULADORA DE RAMOS — internal WhatsApp pricing tool
...
function Calculadora() {
  ...
  const [materials, setMaterials] = React.useState(3.50);
  const [servicePct, setServicePct] = React.useState(35);
  const [marginPct, setMarginPct] = React.useState(20);

  const flowerCost = flowers.reduce((sum, f) => sum + f.qty * f.unit, 0);
  ...
  const updateFlower = (i, field, val) => {
    setFlowers(f => f.map((row, idx) => idx === i ? { ...row, [field]: val } : row));
  };
  ...
  return (
    <div style={{
      ...
      {/* Window chrome */}
      <div style={{
        height:36, background:'#F8F7F2', borderBottom:'1px solid var(--c-line)',
        display:'flex', alignItems:'center', padding:'0 14px', gap:7, flexShrink:0,
      }}>
        <span style={{ width:11, height:11, borderRadius:'50%', background:'#FF5F57' }}/>
        <span style={{ width:11, height:11, borderRadius:'50%', background:'#FEBC2E' }}/>
        <span style={{ width:11, height:11, borderRadius:'50%', background:'#28C840' }}/>
        <span style={{ flex:1, textAlign:'center', fontSize:11, color:'#888', letterSpacing:'0.04em' }}>
          Calculadora · Floristería Selva Garden — herramienta interna
        </span>
        <div style={{ width:33 }}/>
      </div>
      ...
      {/* Header */}
      <div style={{
        padding:'22px 28px 18px', borderBottom:'1px solid var(--c-line)',
        ...
        <div style={{ display:'flex', gap:8 }}>
          ...
          <button style={btnGhost}>Limpiar</button>
        </div>
      </div>
      ...
      <div style={{ flex:1, display:'grid', gridTemplateColumns:'1.4fr 1fr', overflow:'hidden', minHeight:0 }}>
        ...
        <div style={{ padding:'22px 28px', overflowY:'auto', borderRight:'1px solid var(--c-line)' }}>
          {/* AI photo analysis status */}
          <div style={{
            background:'#F5EDD8', border:'1px dashed rgba(181,135,58,0.4)',
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
            <div style={{
              display:'grid', gridTemplateColumns:'1fr 70px 80px 90px 24px',
              gap:8, padding:'0 8px',
              fontSize:9.5, color:'#888', letterSpacing:'0.08em', textTransform:'uppercase', fontWeight:600,
            }}>
            ...
            {flowers.map((f, i) => (
              <div key={i} style={{
                display:'grid', gridTemplateColumns:'1fr 70px 80px 90px 24px', gap:8,
                background:'#FAF9F4', borderRadius:10, padding:'9px 8px', alignItems:'center',
              }}>
        ...
        <div style={{ background:'#FAF9F4', padding:'22px 24px', overflowY:'auto', display:'flex', flexDirection:'column' }}>
          ...
          <div style={{
            ...
            color:'#fff', borderRadius:14, padding:'16px 18px', marginTop:18,
          ...
          <button onClick={() => navigator.clipboard?.writeText(waMsg)} style={{
...
function PriceRow({ l, r, bold }) {
  return (
    <div style={{
      ...
      padding:'9px 0', borderBottom:'1px solid var(--c-line-soft)',
...
function SliderRow({ label, pct, setPct, max=100 }) {
  return (
    <div style={{ marginBottom:12 }}>
      ...
      <input type="range" min="0" max={max} value={pct} onChange={e => setPct(+e.target.value)} style={{
        width:'100%', accentColor:'#1A3C2E', height:4,
      }}/>
    </div>
  );
}
