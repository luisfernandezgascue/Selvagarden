...
function StatCard({ value, label, gold }) {
  return (
    <div style={{ background: '#fff', border: '1px solid #E8EDE9', borderRadius: 12, padding: '14px 16px', flex: 1 }}>
      <p style={{ fontFamily: 'Georgia,serif', fontSize: 26, fontWeight: 700, color: gold ? '#B5873A' : '#1A3C2E', lineHeight: 1, margin: 0 }}>{value}</p>
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: 16, padding: '28px 24px', maxWidth: 360, width: '100%', textAlign: 'center' }}>
        ...
        <h3 style={{ fontFamily: 'Georgia,serif', fontSize: 20, fontWeight: 700, color: '#1A3C2E', marginBottom: 10 }}>Business Central</h3>
        ...
        <button onClick={onClose} style={{ background: '#1A3C2E', color: '#fff', border: 'none', borderRadius: 10, padding: '11px 24px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Entendido</button>
      </div>
    </div>
  );
}
  ...
  return (
    <>
      <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 100, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
        <div onClick={e => e.stopPropagation()} style={{ background: '#F4F6F1', borderRadius: '20px 20px 0 0', padding: '0 0 32px', width: '100%', maxWidth: 640, maxHeight: '90vh', overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
          {/* Handle */}
          ...
          <div style={{ padding: '12px 22px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
              ...
              <h2 style={{ fontFamily: 'Georgia,serif', fontSize: 22, fontWeight: 700, color: '#1A3C2E', margin: 0 }}>{p.cliente_nombre || 'Sin nombre'}</h2>
          ...
          <div style={{ margin: '0 14px 14px', background: '#fff', borderRadius: 12, border: '1px solid #E8EDE9', overflow: 'hidden' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 60px 70px 80px', gap: 8, padding: '8px 14px', background: '#F4F6F1', fontSize: 9.5, color: '#888', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase' }}>
            ...
            {(p.items || []).map((item, i) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 60px 70px 80px', gap: 8, padding: '9px 14px', borderTop: '1px solid #F0F2EE', fontSize: 12 }}>
                ...
                <span style={{ textAlign: 'right', fontWeight: 600, color: '#1A3C2E' }}>${Number(item.subtotal ?? ((item.qty || item.cantidad) * (item.unit || item.precio_unit)) ?? 0).toFixed(2)}</span>
              </div>
            ))}
          </div>
          ...
          <div style={{ margin: '0 14px 14px', background: '#fff', borderRadius: 12, border: '1px solid #E8EDE9', padding: '12px 16px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0 2px', fontSize: 16, fontWeight: 700, color: '#1A3C2E' }}>
              <span>Total</span>
          ...
          {p.notas && (
            <div style={{ margin: '0 14px 14px', background: '#FBF6ED', borderRadius: 12, border: '1px solid rgba(181,135,58,0.2)', padding: '12px 16px' }}>
              <p style={{ fontSize: 10, color: '#B5873A', fontWeight: 700, letterSpacing: '0.08em', marginBottom: 5, textTransform: 'uppercase' }}>Notas</p>
          ...
          <div style={{ margin: '0 14px' }}>
            ...
            >
              ...
              {p.bc_quote_id && <span style={{ fontSize: 10, color: '#2D6A4F' }}>· {p.bc_quote_id}</span>}
            </button>
          </div>
        </div>
      </div>
...
export default function Presupuestos() {
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', background: '#F4F6F1', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ width: 36, height: 36, borderRadius: '50%', border: '3px solid rgba(26,60,46,0.2)', borderTopColor: '#1A3C2E', animation: 'spinSlow 0.9s linear infinite' }}/>
    </div>
  );

  if (authError) return (
    <div style={{ minHeight: '100vh', background: '#F4F6F1', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ background: '#fff', borderRadius: 16, padding: '32px 28px', textAlign: 'center', maxWidth: 340 }}>
        <div style={{ fontSize: 36, marginBottom: 12 }}>🔒</div>
        <h2 style={{ fontFamily: 'Georgia,serif', fontSize: 20, color: '#1A3C2E', marginBottom: 8 }}>Acceso restringido</h2>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F4F6F1', fontFamily: "'Helvetica Neue',Helvetica,Arial,sans-serif" }}>
      {/* Header */}
      <div style={{ background: '#1A3C2E', padding: '20px 24px 18px' }}>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 14, overflowX: 'auto', paddingBottom: 2 }}>
          {['todos', ...Object.keys(ESTADO_CONFIG)].map(e => (
            <button
              ...
              style={{
                background: filtroEstado === e ? '#1A3C2E' : '#fff',
                color: filtroEstado === e ? '#fff' : '#4A4A4A',
                border: '1px solid',
                borderColor: filtroEstado === e ? '#1A3C2E' : '#E8EDE9',
              ...
              return (
                ...
                >
                  ...
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    <p style={{ fontFamily: 'Georgia,serif', fontSize: 18, fontWeight: 700, color: '#1A3C2E', margin: 0 }}>${Number(p.total)}</p>
